...
public class MinHeapImplementation {
		...
		public void bubbleUp(){
			...
			while(pos>0 && mH[parentPos]>mH[pos]){
				int y = mH[pos];
				mH[pos]=mH[parentPos];
				mH[parentPos] = y;
				pos = parentPos;
		...
		public void sinkDown(int k){int a = mH[k];
			...
			if(2*k+2<position && mH[smallest]>mH[2*k+2]){
				smallest = 2*k+2;
			}
			if(smallest!=k){
				swap(k,smallest);
		...
		public static void main(String args[]){
			int arrA [] = {3,2,7,8,4,10,16,12,1};
			System.out.print("Original Array : ");
			for(int i=0;i<arrA.length;i++){
				System.out.print("  " + arrA[i]);
			}
			MinHeapImplementation m = new MinHeapImplementation(arrA.length);
			System.out.print("\nMin-Heap : ");
			m.createHeap(arrA);		
			m.display();
